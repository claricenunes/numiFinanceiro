    ...
    <FadeIn className="px-4 py-5 lg:px-8 lg:py-6 max-w-6xl mx-auto">
      ...
      <div className="rounded-2xl p-5 mb-6" style={{ background: "#131929", border: "1px solid #1E2D45" }}>
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            {summary.totalInvested > 0 && (
              <div className="flex items-center gap-2 mt-1.5">
                <span
                  ...
                  style={{ background: plPositive ? "#34D39922" : "#F8717122", color: plPositive ? "#34D399" : "#F87171" }}
                >
                  {plPositive ? "+" : ""}{formatCurrency(summary.profitLoss)}
                </span>
                <span className="text-sm font-semibold" style={{ color: plPositive ? "#34D399" : "#F87171" }}>
          ...
          <div>
            ...
            <p className="text-xl font-bold text-[#F1F5F9]">{formatCurrency(summary.totalInvested)}</p>
            <p className="text-xs text-[#475569] mt-1">Custo total</p>
          </div>
          <div>
            <p className="text-xs text-[#94A3B8] mb-1">Rentabilidade mês</p>
            <p className="text-xl font-bold" style={{ color: summary.monthlyReturn >= 0 ? "#34D399" : "#F87171" }}>
              {summary.monthlyReturn >= 0 ? "+" : ""}{summary.monthlyReturn.toFixed(2)}%
            </p>
            <p className="text-xs text-[#475569] mt-1 capitalize">{monthLabel}</p>
          </div>
        </div>
      </div>
      ...
      {positions.length === 0 ? (
        <div className="text-center py-20">
          ...
          <p className="text-[#94A3B8] font-medium">Nenhuma posição cadastrada</p>
          <p className="text-sm text-[#475569] mt-1">Adicione seus investimentos para acompanhar a carteira</p>
        </div>
      ) : (
        <>
          <div className="rounded-2xl p-5 mb-6" style={{ background: "#131929", border: "1px solid #1E2D45" }}>
          ...
          <p className="text-xs font-semibold text-[#475569] uppercase tracking-wider mb-3">Posições</p>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                {pos.ticker && <p className="text-sm font-bold text-[#F1F5F9]">{pos.ticker}</p>}
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-0.5 mt-2 pt-2" style={{ borderTop: "1px solid #1E2D45" }}>
            <p className="text-xs text-[#475569]">Qtd: <span className="text-[#94A3B8]">{pos.quantity}</span></p>
            <p className="text-xs text-[#475569]">PM: <span className="text-[#94A3B8]">{formatCurrency(pos.averagePrice)}</span></p>
            {pos.currentPrice && (
              <p className="text-xs text-[#475569]">Atual: <span className="text-[#94A3B8]">{formatCurrency(pos.currentPrice)}</span></p>
            )}
          </div>
        </div>
      </div>
